...
void CFileParserDlg::OnBnClickedButtonAddTime()
{
	...
	//}
	CFile lasfile;
	if (!lasfile.Open(m_strLasPath,CFile::modeRead|CFile::shareDenyNone))
	{
		idxfile.Close();
		//csdfile.Close();
		return;
...
//	Variable_Length_Record lasvariablerecord;
	...
	m_Progress.SetRange32(0,(int)las_time_set.size()-1);
	m_Progress.SetPos(0);
	m_Progress.SetStep(1);
	UpdateData(FALSE);
	BYTE WaveData[40];
	...
	UINT nSize = (UINT)las_time_set.size();
	std::set<double>::iterator iterbegin = las_time_set.begin();
	std::set<double>::iterator iterlast = las_time_set.end(); 
	iterlast--;
	for (std::set<double>::iterator iterset = las_time_set.begin();iterset!=las_time_set.end();iterset++)
	{
		// do find
		int ipos=0;
		double dmintime = (*iterset) - 0.00002;
		double dmaxtime = (*iterset) - 0.000015;
		if ((*iterset)>dndfmax)
			break;
		if ((*iterlast)<dndfmin)
			break;
