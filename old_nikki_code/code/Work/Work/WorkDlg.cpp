...
BEGIN_MESSAGE_MAP(CWorkDlg, CDialog)
	...
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_BUTTON_READ, &CWorkDlg::OnBnClickedButtonRead)
	ON_BN_CLICKED(IDC_BUTTON_START, &CWorkDlg::OnBnClickedButtonStart)
END_MESSAGE_MAP()
...
void CWorkDlg::OnBnClickedButtonRead()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CWorkDlg::OnBnClickedButtonStart()
{
	// TODO: 在此添加控件通知处理程序代码
	CString strOut;
	...
	CString strCont;
	for (UINT j=0;j<m_planarVec[0].size();j++)
	{
		for (UINT i=0;i<m_planarVec.size();i++)
		{
...
void CWorkDlg::InitInput(CString& strInput)
{
...
void CWorkDlg::InsertIntoVec(CString& strContent,int& iRow)
{
...
bool CWorkDlg::CompareAround(int iRow, int iCol)
{
	if (iRow-1>=0 && iCol-1>=0)
	{
		if (175 > m_planarVec[iRow-1][iCol-1])
	...
	if (iRow-1>=0)
	{
		...
		if (175 > m_planarVec[iRow-1][iCol])
	...
	if (iCol-1>=0)
	{
		if (175 > m_planarVec[iRow][iCol-1])
