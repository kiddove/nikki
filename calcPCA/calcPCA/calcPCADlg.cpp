	...
	for (int k = 1; k < 2; k++)
	{
		...
		for (int r = 0; r < result.rows; r++)
		{
			for (int c = 0; c < result.cols; c++)
			{
				...
				pData[index] = ImageReader::GetValue(result, r, c, 0);
			}
		}
...
{
	ImageReader image;
	bool bSuc = image.LoadFile(strFile.GetBuffer(1024));
	strFile.ReleaseBuffer();
	image.CloseFile();
	if (!bSuc)
		return;
	...
	// reshape same data different header, do not release image.m_data
	cv::Mat tobeDone = ReshapeByOpenCV(image.m_data);
	...
	try {
		...
		cv::Mat result = mult.reshape(4, image.m_data.rows);
...
cv::Mat CcalcPCADlg::ReshapeByMe(cv::Mat& mat)
{
	...
	for (int c = 0; c < tobeDone.cols; c++) {
		...
		for (int r = 0; r < tobeDone.rows; r++) {
			float uValue = ImageReader::GetValue(tobeDone, r, c, 0);
...
void CcalcPCADlg::FillBackZero(cv::Mat& nonZero, cv::Mat& tobeDone)
{
	for (int c = 0; c < tobeDone.cols; c++)
	{
		...
		for (int r = 0; r < tobeDone.rows; r++)
		{
			float fValue = ImageReader::GetValue(tobeDone, r, c, 0);
			float ftobeSet = ImageReader::GetValue(nonZero, index, c, 0);
			...
			{
				ImageReader::SetValue(tobeDone, r, c, 0, ftobeSet);
				index++;
			}
