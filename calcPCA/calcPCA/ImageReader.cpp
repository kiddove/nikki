...
bool ImageReader::LoadFile(string strFile)
{
	...
	int iii = getOpenCVType(GDT_Float32, 4);
	m_data.create(iHeight, iWidth, iii);
	...
	for (int i = 1; i <= iChannel; i++)
	{
		...
		if (poBand != NULL)
		{
			...
			Mat fMat;
			fMat.create(iHeight, iWidth, getOpenCVType(GDT_Float32, 1));
			//tobeMerged.push_back(mtt.clone());

			unsigned short* p = (unsigned short*)pafScanline;
			//TRACE(_T("band %d:\n"), i-1);
			for (int ii = 0; ii < iHeight; ii++)
			{
				for (int jj = 0; jj < iWidth; jj++)
				{
					...
					float fValue = (float)mtt.at<unsigned short>(ii, jj);
...
float ImageReader::GetValue(Mat& mat, int r, int c, int b)
{
	if (!mat.isContinuous())
		return 0.0;
	if (r < 0 || r >= mat.rows)
...
void ImageReader::SetValue(Mat& mat, int r, int c, int b, float& fValue)
{
	if (!mat.isContinuous())
		return;
	if (r < 0 || r >= mat.rows)
